...
master
DROP
DATABASE G3P_Coffee1

DROP PROCEDURE IF EXISTS G3P_Coffee1;
GO
...
GO
ALTER TABLE orders ADD customer_name VARCHAR(255);
ALTER TABLE orders ADD customer_email VARCHAR(255);
ALTER TABLE orders ADD customer_phone VARCHAR(20);
ALTER TABLE orders ADD customer_address TEXT;
ALTER TABLE orders ADD first_product_image_url NVARCHAR(255);
...
GO
ALTER TABLE Cart
ADD appliedPromotions NVARCHAR(MAX);
...
-- Create PromotionAction table
CREATE TABLE PromotionAction (
    id INT PRIMARY KEY IDENTITY(1,1),
    promotion_id INT NOT NULL,
    action_type NVARCHAR(50) NOT NULL,
    action_data NVARCHAR(MAX), -- JSON format
    created_at DATETIME DEFAULT (CURRENT_TIMESTAMP),
    updated_at DATETIME DEFAULT (CURRENT_TIMESTAMP),
    FOREIGN KEY (promotion_id) REFERENCES Promotion(id)
);
